...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === '/api/export/list') {
      return json(res, TABLE_NAMES.map((n) => ({ name: n, url: `/api/export/${encodeURIComponent(n)}` })));
    }
    if (p.startsWith('/api/export/')) {
      const name = decodeURIComponent(p.slice('/api/export/'.length));
      const csv = buildTable(db, name);
      if (!csv) return json(res, { error: '該当する書き出しがありません' }, 404);
