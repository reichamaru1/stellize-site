...
export const TABLES = {
  ...
  sns_accounts: T('アカウント設計', {
    order: 'platform, handle',
    search: ['handle', 'name', 'concept', 'target'],
    filters: ['platform'],
    columns: [
      { k: 'platform', label: '媒体', type: 'text', w: 100 },
      { k: 'handle', label: 'ID', type: 'text', w: 140 },
      { k: 'name', label: '表示名', type: 'text', w: 150 },
      { k: 'purpose', label: '目的', type: 'text' },
      { k: 'target', label: 'ターゲット', type: 'text' },
      { k: 'target_pain', label: '相手の困りごと', type: 'text' },
      { k: 'concept', label: 'コンセプト', type: 'text' },
      { k: 'value', label: '持ち帰れるもの', type: 'text' },
      { k: 'tone', label: '話し方', type: 'text' },
      { k: 'pillars', label: '投稿の柱', type: 'text' },
      { k: 'ng', label: 'やらないこと', type: 'text' },
      { k: 'cta', label: '最終的にしてほしいこと', type: 'text' },
      { k: 'kpi', label: '追う指標', type: 'text', w: 120 },
      { k: 'kpi_target', label: '目標', type: 'text', w: 100 },
      { k: 'memo', label: 'メモ', type: 'text' },
    ],
  }),

  sns_posts: T('投稿', {
    order: "CASE status WHEN '案' THEN 0 WHEN '台本' THEN 1 WHEN '予約' THEN 2 ELSE 3 END, "
      + "COALESCE(NULLIF(posted_on,''), planned_on) DESC",
    dateCol: 'planned_on',
    search: ['theme', 'hook', 'caption', 'script', 'memo'],
    filters: ['status', 'platform', 'format', 'pillar', 'account'],
    columns: [
      { k: 'status', label: '状態', type: 'text', w: 80 },
      { k: 'planned_on', label: '予定日', type: 'date', w: 110 },
      { k: 'posted_on', label: '公開日', type: 'date', w: 110 },
      { k: 'platform', label: '媒体', type: 'text', w: 95 },
      { k: 'format', label: '形式', type: 'text', w: 100 },
      { k: 'pillar', label: '柱', type: 'text', w: 120 },
      { k: 'theme', label: 'テーマ', type: 'text' },
      { k: 'hook', label: 'フック', type: 'text' },
      { k: 'reach', label: 'リーチ', type: 'number' },
      { k: 'views', label: '再生', type: 'number' },
      { k: 'likes', label: 'いいね', type: 'number' },
      { k: 'comments', label: 'コメント', type: 'number' },
      { k: 'saves', label: '保存', type: 'number' },
      { k: 'shares', label: 'シェア', type: 'number' },
      { k: 'profile_hits', label: 'プロフ遷移', type: 'number' },
      { k: 'follows', label: 'フォロー増', type: 'number' },
      { k: 'script', label: '台本', type: 'text' },
      { k: 'caption', label: 'キャプション', type: 'text' },
      { k: 'hashtags', label: 'ハッシュタグ', type: 'text' },
  ...
  sns_refs: T('参考事例', {
    order: 'id DESC',
    dateCol: 'posted_on',
    search: ['account', 'theme', 'hook', 'why', 'borrow'],
    filters: ['platform', 'industry', 'format', 'tried', 'account'],
    columns: [
      { k: 'platform', label: '媒体', type: 'text', w: 95 },
      { k: 'account', label: 'アカウント', type: 'text', w: 150 },
      { k: 'industry', label: '業種', type: 'text', w: 120 },
      { k: 'posted_on', label: '投稿日', type: 'date', w: 110 },
      { k: 'format', label: '形式', type: 'text', w: 100 },
      { k: 'theme', label: 'テーマ', type: 'text' },
      { k: 'followers', label: 'フォロワー', type: 'number' },
      { k: 'views', label: '再生', type: 'number' },
      { k: 'likes', label: 'いいね', type: 'number' },
      { k: 'comments', label: 'コメント', type: 'number' },
      { k: 'hook', label: 'フック', type: 'text' },
      { k: 'why', label: 'なぜ伸びたか', type: 'text' },
      { k: 'borrow', label: '取り込む要素', type: 'text' },
      { k: 'tried', label: '試した', type: 'text', w: 90 },
      { k: 'url', label: 'URL', type: 'text' },
      { k: 'memo', label: 'メモ', type: 'text' },
    ],
  }),
